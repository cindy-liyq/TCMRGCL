...
class GCL_Encoder(nn.Module):
    ...
    def forward(self,perturbed_adj=None):
        ...
        all_embeddings = torch.stack(all_embeddings,
                                     dim=1)  # 把3层的嵌入堆叠在一起(39642,3,64)********************tip2------换聚合方式
...
class TCMRGCL(nn.Module):  # hidden_dim=256
    ...
    def fenceng_aggregation(self,sym_emb,herb_emb):
        # 原视图SH图中

        # self.E_s_list[0]=sym_emb#2222222222222222222old
        self.E_s_list[0]=self.pre_train_model.embedding_dict['sym_emb']#2222222222222222222new
        # self.E_h_list[0]=herb_emb#2222222222222222222old
        self.E_h_list[0]=self.pre_train_model.embedding_dict['herb_emb']#2222222222222222222new
    ...
    def _high_order_u_relations(self, herb_emb, sym_emb,adj_ss,adj_hh):
        # sym_embeddings=sym_emb#11111111111111111111 old
        sym_embeddings=self.pre_train_model.embedding_dict['sym_emb']#11111111111111111111 new
        # herb_embeddings=herb_emb#11111111111111111111111111 old
        herb_embeddings=self.pre_train_model.embedding_dict['herb_emb']#11111111111111111111 new
    ...
    def _prompts_generation(self, herb_emb, sym_emb,adj_ss,adj_hh):
        sym_embed,herb_embed = self._high_order_u_relations(herb_emb, sym_emb,adj_ss,adj_hh)
    ...
    def _prompts_u_embeddings_fusion(self, prompts_sym, sym_emb,prompts_herb,herb_emb,fenceng_sym_embedding,fenceng_herb_embedding):
    ...
    def fast_evaluation(self,epoch,bestPerformance,bestPerformance_data,current_sym_emb,current_herb_emb,best_sym_emb,best_herb_emb):
        ...
        print('-' * 120)
        print("output_result_len:",len(bestPerformance_data[1]))
    ...
    def train(self):
        ...
        best_herb_emb = None
        self._pre_train()
        model=self.pre_train_model.to(self.device)
        ...
        for epoch in range(self.epoch):
            ...
            for n, batch in enumerate(self.origin_data['train_loader']):
                ...
                # todo:整合embedding
                prompted_sym_emb,prompted_herb_emb = self._prompts_u_embeddings_fusion(prompts_sym, sym_emb,prompts_herb,herb_emb,fenceng_sym_embedding,fenceng_herb_embedding)
            ...
            print("bestPerformance_data[1]:", len(bestPerformance_data[1]))
            #利用pickle将两个embedding打包，我这边可以直接读取embeding
            import pickle
            with open("./best_sym_emb.pkl",'wb') as file_1:
                pickle.dump(best_sym_emb,file_1)
            with open("./best_herb_emb.pkl",'wb') as file_2:
